...
class HistoryGeneratorService {
  ...
  async execute(companyId: string, clientKey: string, dateFrom: string, dateTo: string) {
    ...
    const historyMessage = await historyMessageService.execute(await token.accessToken, companyId, historyContact);
    ...
    historyMessage.forEach(async (item: any) => {
      ...
      if (await dir.verifyDir(`../temp/`) === false) {
        dir.createDir(`../temp/`);
      }
      if (await dir.verifyDir(`../temp/${year}`) === false) {
        dir.createDir(`../temp/${year}`);
      }

      if (await dir.verifyDir(`../temp/${year}/${month}`) === false) {
        dir.createDir(`../temp/${year}/${month}`);
      }

      if (await dir.verifyDir(`../temp/${year}/${month}/${day}`) === false) {
        dir.createDir(`../temp/${year}/${month}/${day}`);
      }

      if (await dir.verifyDir(`../temp/${year}/${month}/${day}/${item.protocol}`) === false) {
        dir.createDir(`../temp/${year}/${month}/${day}/${item.protocol}`);
      }
